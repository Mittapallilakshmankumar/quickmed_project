...
//   // ✅ ADD HERE//there are backned
//   const [doctors, setDoctors] = useState([]);
//   const [loadingDoctors, setLoadingDoctors] = useState(true);
//     // ✅ STEP 4 HERE there sre backend 
...
//   useEffect(() => {
//     fetch("http://127.0.0.1:8000/api/doctor/list/")
//       .then(res => res.json())
//       .then(data => {
//         setDoctors(data);
...
//   // ✅ STEP 5: fetch doctor time slots (USER SIDE)
// useEffect(() => {
//   fetch("http://127.0.0.1:8000/api/doctor/user/timeslots/")
//     .then(res => res.json())
//     .then(data => {
//       setDoctorSlots(data);
//     })
...
//   // 🔹 STEP 3: split doctors by specialty (USER SIDE)
// const pediatricDoctors = doctors.filter(
//   d => d.specialty === "pediatric"
// );

// const pregnancyDoctors = doctors.filter(
//   d => d.specialty === "pregnancy"
// );

// const generalDoctors = doctors.filter(
//   d => d.specialty === "general"
// );
...
//   const [showGeneralDoctors, setShowGeneralDoctors] = useState(false);
//   const [bookedAppointments, setBookedAppointments] = useState([]);
//   // ✅ FINAL LIST OF DOCTORS TO DISPLAY (FIX WHITE PAGE)
...
//   const [doctors, setDoctors] = useState([]);
//   const [loadingDoctors, setLoadingDoctors] = useState(true);
//     // ✅ STEP 4 HERE there sre backend  for the slots to see in the frontend  docto sslots 
//   const [doctorSlots, setDoctorSlots] = useState([]);
//   const slotsByDoctor = useMemo(() => {
//   const map = {};
//   doctorSlots.forEach(slot => {
//      const doctorId = slot.doctor.id; // ✅ correct
//     if (!map[slot.doctor]) {
//       map[slot.doctor] = [];
//     }
//     if (slot.is_available && !slot.is_booked) {
...
const TimeSlotsModal = memo(({
  ...
  //added because to see real slots what coming from the docto side 
  const availableDates = useMemo(() => {
  if (!selectedDoctor || !selectedDoctor.availableSlots) return [];

  const dates = selectedDoctor.availableSlots.map(s => s.date);
...
//thser are used to see the get available slots when docto aded the  doctor side these coming here 
const getAvailableTimeSlots = useCallback(() => {
  if (!selectedDoctor || !selectedDoctor.availableSlots || !selectedAppointmentDate) return [];

  return selectedDoctor.availableSlots
    .filter(slot => slot.date === selectedAppointmentDate)
    .map(slot => ({
      id: slot.id,
  ...
  return (
    <DateButton
      ...
      isSelected={date === selectedAppointmentDate}
      onSelect={onDateSelect}
    />
  );
})}
            ...
            {/* Time Selection */}
            {/* it ised to see th ereal time slotd in the user side when the doctor added the slot  */}
{selectedAppointmentDate && (
  <div style={timeSelectionStyle}>
    <label style={{ color: '#4F6F6B', fontSize: '0.9rem', marginBottom: '0.5rem', display: 'block' }}>
      Select Time Slot:
      ...
      <div style={timeSlotsGridStyle}>
        {availableTimeSlots.map(slot => (
          <TimeSlotButton
            key={slot.id}
            time={slot.label}
            isSelected={slot.label === selectedAppointmentTime}
            onSelect={() => onTimeSelect(slot.label)}
          />
        ))}
      </div>
    )}
