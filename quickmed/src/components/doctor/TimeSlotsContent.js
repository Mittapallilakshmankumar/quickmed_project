...
const TimeSlotsContent = ({ state, actions }) => {
  ...
  try {
    await fetch(
      `http://127.0.0.1:8000/api/doctor/timeslots/${slotId}/delete/`,
      {
        method: "DELETE",
        headers: {
  ...
  try {
    const response = await fetch(
      `http://127.0.0.1:8000/api/doctor/timeslots/${slotId}/toggle/`,
      {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("access_token")}`,
 ...
 useEffect(() => {
  const fetchDoctorSlots = async () => {
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/api/doctor/timeslots/",
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("access_token")}`,
          },
  ...
  const payload = {
    ...
    end_time: `${end.getHours().toString().padStart(2, "0")}:${end
      .getMinutes()
      .toString()
      .padStart(2, "0")}`,
    duration: slotDuration,
        ...
        <div style={styles.slotInfo}>
          ...
          <div style={styles.slotStatus}>
            <span style={{
              ...
              {slot.isBooked ? 'Booked' : slot.is_available ? 'Available' : 'Busy'}
            </span>
          </div>
        </div>
